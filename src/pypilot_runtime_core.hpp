...
using pypilot_data_model::RuntimeValueMetadata;
using pypilot_data_model::RuntimeValueType;
...
static inline const char* runtime_protocol_type_name(RuntimeValueType type) {
    switch (type) {
    case RuntimeValueType::boolean: return "bool";
    case RuntimeValueType::number: return "number";
    case RuntimeValueType::integer: return "number";
    case RuntimeValueType::string_value: return "string";
    case RuntimeValueType::mode: return "string";
    case RuntimeValueType::pilot_name: return "string";
    }
    return "unknown";
}

static inline bool append_json_value(char* out, size_t out_size, const RuntimeValueMetadata& value) {
    char item[192]{};
...
static inline const RuntimeValueMetadata* runtime_metadata_values(size_t& count) {
    return pypilot_data_model::core_runtime_values(count);
}

static inline const RuntimeValueMetadata* find_runtime_value_metadata(const char* name) {
    if (!name) return nullptr;
    size_t count = 0;
    const RuntimeValueMetadata* values = runtime_metadata_values(count);
...
static inline bool format_runtime_value(const PypilotRuntimeState& state,
                                        const RuntimeValueMetadata& value,
                                        char* out,
                                        size_t out_size) {
    switch (value.type) {
    ...
    case RuntimeValueType::string_value: {
        const char* text = "";
    ...
    case RuntimeValueType::pilot_name: {
...
public:
    ...
    bool value_exists(const char* name) const {
        return find_runtime_value_metadata(name) != nullptr;
    }

    bool writable(const char* name) const {
        const RuntimeValueMetadata* value = find_runtime_value_metadata(name);
