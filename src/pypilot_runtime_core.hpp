...
static inline bool append_json_value(char* out, size_t out_size, const DataDictionaryEntry& value) {
    ...
    snprintf(item, sizeof(item), "\"%s\":{\"type\":\"%s\",\"writable\":%s}",
             value.name,
             pypilot_data_model::data_value_type_name(value.type),
             value.writable ? "true" : "false");
    if (out[0] != '\0' && out[pypilot_data_model::data_text_length(out) - 1] != '{') {
        if (!pypilot_data_model::append_data_text(out, out_size, ",")) return false;
    }
    return pypilot_data_model::append_data_text(out, out_size, item);
}
...
static inline bool apply_runtime_set(PypilotRuntimeState& state,
    ...
    case DataValueType::boolean: {
        bool parsed = false;
        if (!pypilot_data_model::parse_data_bool(payload, parsed)) { snprintf(error, error_size, "error=bad bool\n"); return false; }
    ...
    case DataValueType::integer: {
        double parsed = 0.0;
        if (!pypilot_data_model::parse_data_number(payload, parsed)) { snprintf(error, error_size, "error=bad number\n"); return false; }
    ...
    case DataValueType::string_value: {
        char text[128]{};
        if (!pypilot_data_model::strip_optional_data_quotes(payload, text, sizeof(text))) { snprintf(error, error_size, "error=bad string\n"); return false; }
    ...
    case DataValueType::mode: {
        char text[32]{};
        pypilot_data_model::AutopilotMode mode{};
        if (!pypilot_data_model::strip_optional_data_quotes(payload, text, sizeof(text)) || !pypilot_data_model::autopilot_mode_from_name(text, mode)) { snprintf(error, error_size, "error=bad autopilot mode\n"); return false; }
    ...
    case DataValueType::pilot_name: {
        char text[32]{};
        pypilot_data_model::PilotName pilot{};
        if (!pypilot_data_model::strip_optional_data_quotes(payload, text, sizeof(text)) || !pypilot_data_model::pilot_from_name(text, pilot)) { snprintf(error, error_size, "error=bad pilot\n"); return false; }
...
public:
    ...
    bool write_values_catalog(char* out, size_t out_size) const {
        ...
        out[0] = '\0';
        if (!pypilot_data_model::append_data_text(out, out_size, "values={")) return false;
        size_t count = 0;
        ...
        return pypilot_data_model::append_data_text(out, out_size, "}\n");
    }
