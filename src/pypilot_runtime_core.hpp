...
public:
    ...
    bool apply_set(const char* name, const char* payload, char* error, size_t error_size) {
        if (!value_exists(name)) { snprintf(error, error_size, "error=unknown value %s\n", name ? name : ""); return false; }
        if (!writable(name)) { snprintf(error, error_size, "error=value not writable %s\n", name); return false; }

        if (strcmp(name, "ap.enabled") == 0 || strcmp(name, "servo.engaged") == 0) {
            bool parsed = false;
            if (!parse_bool_text(payload, parsed)) { snprintf(error, error_size, "error=bad bool\n"); return false; }
            if (strcmp(name, "ap.enabled") == 0) state_.ap.enabled.value = parsed;
            else state_.servo.engaged.value = parsed;
            return true;
        }
...
inline bool PypilotRuntimeProtocol::format_value(const char* name, char* out, size_t out_size) const {
...
inline bool PypilotRuntimeProtocol::write_values_catalog(char* out, size_t out_size) const {
...
inline bool PypilotRuntimeProtocol::append_value_catalog(char* out, size_t out_size) const {
